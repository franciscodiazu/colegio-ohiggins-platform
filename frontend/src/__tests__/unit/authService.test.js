...
describe('resetUserPassword', () => {
  it('cambia la contraseña correctamente (mock local)', async () => {
    bffClient.post.mockResolvedValue({ data: {} });
    const reset = await resetUserPassword({ email: 'ana@profesor.cl', newPassword: 'nuevaClave456' });
    expect(reset.ok).toBe(true);
  });

  it('falla si el correo no existe', async () => {
    bffClient.post.mockRejectedValue({
      response: { status: 400, data: { error: 'No encontramos una cuenta asociada a ese correo.' } },
    });
    const result = await resetUserPassword({ email: 'noexiste@profesor.cl', newPassword: 'nueva123' });
    expect(result.ok).toBe(false);
  });

  it('falla si la nueva contraseña está vacía', async () => {
    const result = await resetUserPassword({ email: 'ana@profesor.cl', newPassword: '' });
    expect(result.ok).toBe(false);
  });

  it('falla si el correo es de dominio externo', async () => {
    const result = await resetUserPassword({ email: 'ana@gmail.com', newPassword: 'nueva123' });
    expect(result.ok).toBe(false);
  });
});
