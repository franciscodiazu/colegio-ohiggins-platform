...
export default function Evaluaciones() {
  ...
  const evaluationsById = useMemo(() => {
    return evaluaciones.reduce((acc, ev) => { acc[ev.id] = ev; return acc; }, {});
  }, [evaluaciones]);
  ...
  // ── Filtrado dinámico: estudiantes según la evaluación seleccionada ────────
  // Solo muestra alumnos del mismo curso que la evaluación elegida.
  ...
  // ── IDs de estudiantes que ya tienen nota en la evaluación seleccionada ───
  ...
  const studentsAlreadyGraded = useMemo(() => {
    ...
    return new Set(
      calificaciones
        .filter((g) => g.evaluationId === evalId)
        .map((g) => g.studentId),
    );
  ...
  // ── Helper: enriquecer filas de calificaciones ─────────────────────────────

  const mapGradeRows = (grades) => {
  ...
  const handleCrearEvaluacion = (e) => {
    ...
    evaluationsMockService
      .createEvaluation(evaluationForm)
      .then((created) => {
      ...
      .catch((err) => {
        setCreateEvalFeedback({ error: err.message || 'No se pudo registrar la evaluación.', success: '' });
      });
  };
  ...
  const handleSelectEvaluationForEdit = (evaluationId) => {
    ...
    if (!selected) return;
    setEditEvaluationForm({
      nombre: selected.nombre,
      curso: selected.curso,
      fecha: selected.fecha,
      ponderacion: String(selected.ponderacion),
  ...
  const handleActualizarEvaluacion = (e) => {
    ...
    if (!selectedEvaluationId) {
      setEditEvalFeedback({ error: 'Selecciona una evaluación para actualizar.', success: '' });
      return;
    }

    evaluationsMockService
      .updateEvaluation(selectedEvaluationId, editEvaluationForm)
      .then((updated) => {
        if (!updated) {
          setEditEvalFeedback({ error: 'No encontramos la evaluación seleccionada.', success: '' });
          return;
        }
      ...
      .catch((err) => {
        setEditEvalFeedback({ error: err.message || 'No se pudo actualizar la evaluación.', success: '' });
      });
  };
  ...
  const handleRegistrarCalificacion = (e) => {
    ...
    evaluationsMockService
      ...
      .catch((err) => {
        setGradeFeedback({ error: err.message || 'No se pudo registrar la calificación.', success: '' });
      });
  };
  ...
  const handleQueryByStudent = (e) => {
    ...
    if (!queryStudentId) {
      setQueryFeedback({ error: 'Selecciona un estudiante para consultar sus calificaciones.', success: '' });
      return;
    }

    evaluationsMockService
      .listGradesByStudent(queryStudentId)
      .then((records) => {
      ...
      .catch(() => {
        setQueryFeedback({ error: 'No se pudieron consultar las calificaciones por estudiante.', success: '' });
      });
  };
  ...
  const handleQueryEvaluationsByCourse = (e) => {
    ...
    if (!queryCourse) {
      setQueryFeedback({ error: 'Selecciona un curso para consultar evaluaciones.', success: '' });
      return;
    }

    evaluationsMockService
      .listEvaluationsByCourse(queryCourse)
      .then((records) => {
        setCourseQueryRows(records);
        setQueryFeedback({ error: '', success: `${records.length} evaluación(es) encontrada(s).` });
      })
      .catch(() => {
        setQueryFeedback({ error: 'No se pudieron consultar evaluaciones por curso.', success: '' });
      });
  };
    ...
    >
      ...
      <div className="summary-strip">
        <div className="summary-item">
          ...
          <p className="summary-item__value">{evaluaciones.length}</p>
        </div>
        <div className="summary-item">
          <p className="summary-item__label">Notas registradas</p>
          <p className="summary-item__value">{calificaciones.length}</p>
        </div>
        <div className="summary-item">
          <p className="summary-item__label">Promedio general</p>
          <p className="summary-item__value">{promedioNotas}</p>
        </div>
      </div>

      {/* ════════════════════════════════════════════
          SECCIÓN 1 — GESTIÓN DE EVALUACIONES
      ════════════════════════════════════════════ */}
          ...
          <form className="module-form" onSubmit={handleCrearEvaluacion}>
            ...
            <div className="form-row form-row--2">
              ...
              <div className="field-group">
                ...
                <input
                  ...
                  max="100"
                  className="field-control"
                  value={evaluationForm.ponderacion}
            ...
            <div className="field-group">
              ...
              <input
                ...
                value={evaluationForm.descripcion}
        ...
        <LayoutCard className="layout-card--split module-panel">
          ...
          {evaluaciones.length === 0 && (
            <p className="asistencia-hint">
              ⚠️ Primero registra una evaluación para poder editarla.
            </p>
          )}
          ...
          <form className="module-form" onSubmit={handleActualizarEvaluacion}>
            <div className="field-group">
              ...
              >
                ...
                {evaluaciones.map((ev) => (
                  <option key={ev.id} value={ev.id}>{ev.nombre} ({ev.curso})</option>
                ))}
              </select>
            </div>
            ...
            <div className="field-group">
              ...
              <input
                ...
                type="text"
                className="field-control"
                value={editEvaluationForm.nombre}
                disabled={!selectedEvaluationId}
                onChange={(e) => setEditEvaluationForm((prev) => ({ ...prev, nombre: e.target.value }))}
              />
            </div>
            ...
            <div className="field-group">
              ...
              <select
                id="edit-eval-course"
                className="field-control"
                value={editEvaluationForm.curso}
                disabled={!selectedEvaluationId}
                onChange={(e) => setEditEvaluationForm((prev) => ({ ...prev, curso: e.target.value }))}
              >
                <option value="">-- Selecciona un curso --</option>
                {courses.map((course) => (
                  <option key={course} value={course}>{course}</option>
            ...
            <div className="form-row form-row--2">
              <div className="field-group">
                ...
                <input
                  ...
                  type="date"
                  className="field-control"
                  value={editEvaluationForm.fecha}
                  disabled={!selectedEvaluationId}
                  onChange={(e) => setEditEvaluationForm((prev) => ({ ...prev, fecha: e.target.value }))}
                />
              </div>
              ...
              <div className="field-group">
                ...
                <input
                  ...
                  max="100"
                  className="field-control"
                  value={editEvaluationForm.ponderacion}
                  disabled={!selectedEvaluationId}
                  onChange={(e) => setEditEvaluationForm((prev) => ({ ...prev, ponderacion: e.target.value }))}
                />
              </div>
            </div>
            ...
            <div className="field-group">
              ...
              <input
                ...
                type="text"
                className="field-control"
                value={editEvaluationForm.descripcion}
                disabled={!selectedEvaluationId}
                onChange={(e) => setEditEvaluationForm((prev) => ({ ...prev, descripcion: e.target.value }))}
              />
            </div>
      ...
      {/* ════════════════════════════════════════════
          SECCIÓN 2 — LISTADO DE EVALUACIONES
      ════════════════════════════════════════════ */}
      ...
      <section className="table-panel">
        <table className="app-table">
          ...
          <tbody>
            {evaluaciones.length === 0 ? (
              <tr>
                <td colSpan="4" className="empty-state-cell">No hay evaluaciones registradas aún.</td>
              </tr>
            ) : (
              evaluaciones.map((ev) => (
                <tr key={ev.id}>
      ...
      {/* ════════════════════════════════════════════
          SECCIÓN 3 — REGISTRAR CALIFICACIÓN
      ════════════════════════════════════════════ */}
      <div className="asistencia-section-label section-title--spaced">
      ...
      <LayoutCard className="layout-card--split module-panel">
        ...
        {evaluaciones.length === 0 && (
          <p className="asistencia-hint">
            ⚠️ Primero registra una evaluación para poder ingresar calificaciones.
          </p>
        )}
        ...
        <form className="module-form" onSubmit={handleRegistrarCalificacion}>
          <div className="field-group">
            ...
            <select
              id="grade-eval"
              className="field-control"
              value={gradeForm.evaluationId}
              onChange={(e) =>
                setGradeForm((prev) => ({
                  ...prev,
                  evaluationId: e.target.value,
                  studentId: '', // resetear estudiante al cambiar evaluación
                }))
              }
            >
            ...
            <label className="field-label" htmlFor="grade-student">
              ...
              {gradeForm.evaluationId && (
                <span className="asistencia-hint-inline">
                  {' '}— solo se muestran alumnos del curso {evaluationsById[Number(gradeForm.evaluationId)]?.curso}
                </span>
              )}
            </label>
            <select
              ...
              disabled={!gradeForm.evaluationId}
              onChange={(e) => setGradeForm((prev) => ({ ...prev, studentId: e.target.value }))}
            >
              <option value="">
                {gradeForm.evaluationId
                  ? studentsForSelectedEvaluation.length === 0
              ...
              {studentsForSelectedEvaluation.map((est) => (
                <option
                  key={est.id}
                  value={est.id}
                  disabled={studentsAlreadyGraded.has(est.id)}
                >
                  {est.nombre}
                  {studentsAlreadyGraded.has(est.id) ? ' (ya calificado)' : ''}
                </option>
              ))}
            </select>
          </div>
          ...
          <button
            ...
            disabled={!gradeForm.evaluationId || !gradeForm.studentId}
          >
